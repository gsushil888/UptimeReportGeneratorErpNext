...
public class MemoryReportGenerator3 {
	...
	private MemoryReportGenerator3() {
	}
	...
	private static void addCurrentMemoryTable(Document document, List<MemoryUsageRecord> records)
		...
		addStyledSubtitleSectionHeader(document, SUBTITLE_HEADER_CURRENT_MEMORY_DETAILS);

		PdfPTable table = new PdfPTable(9);
		table.setWidthPercentage(100);
		table.setSpacingBefore(5f);
		table.setSpacingAfter(5f);
		table.setWidths(new float[] { 2f, 1.2f, 1f, 1f, 1f, 1f, 1f, 1f, 1.2f });

		// Header Row 1
		table.addCell(createMemorySpanHeaderCell("IP Address", 1));
		table.addCell(createMemorySpanHeaderCell("Size", 1));
		table.addCell(createMemorySpanHeaderCell("Actual Memory", 3));
		table.addCell(createMemorySpanHeaderCell("Memory (Cached & Buffer Included)", 3));
		table.addCell(createMemorySpanHeaderCell("Cached", 1));

		// Header Row 2
		table.addCell(createMemoryHeaderCell(""));
		table.addCell(createMemoryHeaderCell(""));
		table.addCell(createMemoryHeaderCell("Used"));
		table.addCell(createMemoryHeaderCell("Free"));
		table.addCell(createMemoryHeaderCell("Used (%)"));
		table.addCell(createMemoryHeaderCell("Used"));
		table.addCell(createMemoryHeaderCell("Free"));
		table.addCell(createMemoryHeaderCell("Used (%)"));
		...
		if (records.isEmpty()) {
			table.addCell(createNoDataCell("No memory usage data available", 9));
		} else {
			for (MemoryUsageRecord rec : records) {
				...
				table.addCell(createMemoryDataCell(rec.getMemTotal()));
				table.addCell(createMemoryDataCell(rec.getMemActualUsed()));
				table.addCell(createMemoryDataCell(rec.getMemActualFree()));
				table.addCell(createHighlightedDataCell(rec.getMemActualUsedPct()));
				table.addCell(createMemoryDataCell(rec.getMemPhyUsed()));
				table.addCell(createMemoryDataCell(rec.getMemPhyFree()));
				table.addCell(createHighlightedDataCell(rec.getMemPhyUsedPct()));
				table.addCell(createMemoryDataCell(rec.getCachedTotal()));
			}
		}
	...
	private static void addDetailTable(Document document, Map<String, List<MemoryDetailRecord>> recordMap)
		...
		if (recordMap.isEmpty()) {
			// Create table with headers even if no data exists
		...
		for (Map.Entry<String, List<MemoryDetailRecord>> entry : recordMap.entrySet()) {
			...
			Paragraph hostPara = new Paragraph("Hostname: " + hostname,
			...
			if (entry.getValue().isEmpty()) {
				table.addCell(createNoDataCell("No data for this host", 3));
			} else {
				for (MemoryDetailRecord rec : entry.getValue()) {
	...
	private static PdfPCell createHighlightedDataCell(String text) {
		double value = 0;
		try {
	...
	private static PdfPCell createHighlightedDataCell(double value) {
		return createHighlightedDataCell(DECIMAL_FORMAT.format(value));
	}
