...
import com.sushil.elasticsearch.dao.backup.BackupReportDAO;
...
public class BackupReportGenerator {
	...
	public static void addBackupSection(ElasticsearchClient client, Document document,
...
//            System.out.println("Summaries => "+ summaries);
			...
			TreeSet<String> dateSet = summaries.stream().map(BackupSummary::getDate)
	...
	private static Map<String, Map<String, List<String>>> buildBackupDataMap(List<BackupSummary> summaries) {
		...
		for (BackupSummary summary : summaries) {
			backupData.computeIfAbsent(summary.getBackupType(), k -> new HashMap<>())
					.computeIfAbsent(summary.getDate(), k -> new ArrayList<>()).add(summary.getTimestamp());
		}
		return backupData;
	}

	private static PdfPTable buildBackupTable(TreeSet<String> dateSet,
			Map<String, Map<String, List<String>>> backupData) throws DocumentException {
	...
	private static void addBackupDataRows(PdfPTable table, TreeSet<String> dateSet,
			Map<String, Map<String, List<String>>> backupData) {
		for (String backupType : BACKUP_TYPES) {
			table.addCell(createDataCell(backupType.replace("_", " ").toUpperCase()));

			for (String date : dateSet) {
				List<String> timestamps = backupData.getOrDefault(backupType, Collections.emptyMap()).getOrDefault(date,
