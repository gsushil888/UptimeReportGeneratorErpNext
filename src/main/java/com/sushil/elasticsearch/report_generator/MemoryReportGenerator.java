...
public class MemoryReportGenerator {
	...
	// ❌ REMOVED global ENVIRONMENT_PREFIX (was causing issue)

	private static final String SECTION_HEADER_MEMORY_USAGE = "Section C: MEMORY USAGE";

	private static String SUBTITLE_HEADER_CURRENT_MEMORY_DETAILS =
			"1: Current MEMORY Usage Details";

	private static String SUBTITLE_HEADER_MEMORY_EXCEEDS =
			"2. Time range where the MEMORY usage exceeded the 20% threshold continuously for over 5 minutes.";

	private static String SUBTITLE_HEADER_MEMORY_ALL_DETAILS =
			"3. Timestamp when MEMORY usage exceeded the 20% threshold";
	...
	public static void addMemorySection(
		...
		byte[] currentMemoryBytes =
				Files.readAllBytes(Paths.get(jsonFilePathMap.get("current_memory")));

		byte[] memoryBytes =
				Files.readAllBytes(Paths.get(jsonFilePathMap.get("memory")));
		...
		addCurrentMemoryTable(document, usageRecords, excludedEnvPrefix);
		...
		addThresholdTable(document, thresholdRecords, excludedEnvPrefix);
		...
		addDetailTable(document, detailRecords, excludedEnvPrefix);
	}
	...
	private static double detectMemoryThreshold(InputStream jsonStream) throws Exception {
		...
		for (JsonNode condition : rootNode.at("/query/bool/should")) {
			threshold = Math.max(
					threshold,
					condition.at("/range/system.memory.actual.used.pct/gte").asDouble(0.0)
			);
		}

		double thresholdPct = threshold * 100;

		SUBTITLE_HEADER_MEMORY_EXCEEDS =
				SUBTITLE_HEADER_MEMORY_EXCEEDS.replace(
						"20%", DECIMAL_FORMAT.format(thresholdPct) + "%");

		SUBTITLE_HEADER_MEMORY_ALL_DETAILS =
				SUBTITLE_HEADER_MEMORY_ALL_DETAILS.replace(
						"20%", DECIMAL_FORMAT.format(thresholdPct) + "%");
	...
	// ================== CURRENT MEMORY ==================
	private static void addCurrentMemoryTable(
			Document document,
			List<MemoryUsageRecord> records,
			String excludedEnvPrefix) throws DocumentException {

		addStyledSubtitleSectionHeader(
				document,
				withTimestamp(SUBTITLE_HEADER_CURRENT_MEMORY_DETAILS));
		...
		if (records != null) {
			for (MemoryUsageRecord rec : records) {
				...
				String hostname =
						rec.getHostname() != null ? rec.getHostname() : "N/A";
	...
	private static void addThresholdTable(
			Document document,
			List<MemoryThresholdRecord> records,
			String excludedEnvPrefix) throws DocumentException {
	...
	private static void addDetailTable(
			Document document,
			Map<String, List<MemoryDetailRecord>> recordMap,
			String excludedEnvPrefix) throws DocumentException {
		...
		for (Map.Entry<String, List<MemoryDetailRecord>> entry : recordMap.entrySet()) {
			...
			Paragraph hostPara = new Paragraph(
	...
	private static PdfPCell createNoDataCell(String message, int colspan) {
		PdfPCell cell =
				new PdfPCell(new Phrase(message,
						FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11)));
		cell.setColspan(colspan);
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
