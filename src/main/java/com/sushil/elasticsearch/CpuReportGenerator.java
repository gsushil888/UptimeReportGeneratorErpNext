...
public class CpuReportGenerator {
	...
	private static void addCurrentCpuTable(Document document, List<CpuUsageRecord> records, double threshold)
		...
		addStyledSubtitleSectionHeader(document, withTimestamp(SUBTITLE_HEADER_CURRENT_CPU_DETAILS));
	...
	private static void addDetailTable(Document document, Map<String, List<CpuDetailRecord>> detailRecords,
		...
		for (Map.Entry<String, List<CpuDetailRecord>> entry : detailRecords.entrySet()) {
			...
			PdfPCell ipCell = new PdfPCell(
					new Phrase("IP Address: " + entry.getKey(), FontFactory.getFont(FontFactory.HELVETICA_BOLD)));
	...
	private static double detectCpuThreshold(InputStream cpuStream) throws Exception {
		...
		for (JsonNode condition : rootNode.at("/query/bool/should")) {
			...
			userCpu = Math.max(userCpu, range.at("/system.cpu.user.norm.pct/gte").asDouble(0.0));
			systemCpu = Math.max(systemCpu, range.at("/system.cpu.system.norm.pct/gte").asDouble(0.0));
		}

		double threshold = Math.max(userCpu, systemCpu) * 100;

		SUBTITLE_HEADER_CPU_EXCEEDS = SUBTITLE_HEADER_CPU_EXCEEDS.replace("20%", String.format("%.0f%%", threshold));
	...
	private static PdfPTable createTableWithHeaders(List<String> headers, float[] widths) throws DocumentException {
		...
		for (String h : headers) {
			PdfPCell c = new PdfPCell(new Paragraph(h, FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
	...
	private static void addNoDataRow(PdfPTable table, int cols, String msg) {
		PdfPCell c = new PdfPCell(new Paragraph(msg, FontFactory.getFont(FontFactory.HELVETICA_OBLIQUE, 11)));
		c.setColspan(cols);
		c.setHorizontalAlignment(Element.ALIGN_CENTER);
		c.setPadding(5f);
		table.addCell(c);
